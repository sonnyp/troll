...
export function expandRelativePaths(import_map, containing_folder) {
    for (const [k, v] of Object.entries(import_map.imports)) {
        ...
        if (v.startsWith("./") || v.startsWith("../")) {
            import_map.imports[k] = containing_folder.resolve_relative_path(v).get_path();
            if (ends_with_slash) {
                import_map.imports[k] += "/";
            }
        } else if (v.startsWith("file:///./") || v.startsWith("file:///../")) {
            import_map.imports[k] = `file://${containing_folder.resolve_relative_path(v.substring(8)).get_path()}`;
            if (ends_with_slash) {
                import_map.imports[k] += "/";
            }
        } else {
...
export function rewriteImport(import_map, source, parse_info) {
